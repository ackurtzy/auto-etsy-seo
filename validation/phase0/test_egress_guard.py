...
class EgressGuardTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        unsigned = CanaryPermit(
            ...
            gate_reference="sha256:synthetic-gate",
            method="GET",
        ...
        self.approved = ApprovedCanary(
            status="passed",
            environment=self.permit.environment,
            gate_reference=self.permit.gate_reference,
            method=self.permit.method,
            shop_id=self.permit.shop_id,
            listing_id=self.permit.listing_id,
            capability=self.permit.capability,
            expires_at=(datetime.now(timezone.utc) + timedelta(minutes=5)).isoformat(),
        )

    def guard(self) -> EgressGuard:
        return EgressGuard(signing_key=self.key, approved_canary=self.approved)
